...
import { BookOpen, CheckCircle2, ChevronRight, Clock, Award, AlertCircle, HelpCircle } from 'lucide-react';

// Simulasi 30 Soal
const QUESTIONS = Array.from({ length: 30 }, (_, i) => ({
  id: i + 1,
  question: `Ini adalah pertanyaan nomor ${i + 1} untuk menguji kompetensi dasar siswa SMK Prima Unggul. Apa jawaban yang paling tepat?`,
  options: ['Opsi Jawaban A', 'Opsi Jawaban B', 'Opsi Jawaban C', 'Opsi Jawaban D'],
  correctAnswer: 0 // Misal A selalu benar untuk demo
}));
    ...
    return (
      <div className="max-w-3xl mx-auto py-12 px-6">
        ...
        >
          ...
          <h2 className="text-4xl font-black text-slate-900 mb-4 uppercase tracking-tight">Hasil Ujian Anda</h2>
          ...
          <div className="grid grid-cols-2 gap-6 mb-12">
            ...
            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Status KKM (50)</p>
        ...
        <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-slate-800 uppercase tracking-tight">Pertanyaan Ujian</h4>
        ...
        <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
          ...
          <div className="grid grid-cols-5 gap-2">
            {QUESTIONS.map((_, i) => (
              <button
                ...
                className={`aspect-square rounded-xl flex items-center justify-center text-[11px] font-black border-2 transition-all ${
                  currentQuestion === i 
          ...
          <div className="mt-8 pt-6 border-t border-slate-50 space-y-3">
             <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
               <div className="w-3 h-3 bg-red-600 rounded-sm"></div> Sekarang
             </div>
             <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
               <div className="w-3 h-3 bg-green-500 rounded-sm"></div> Sudah Diisi
             </div>
             <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
               <div className="w-3 h-3 bg-slate-100 rounded-sm"></div> Belum Diisi
             </div>
          </div>
        </div>

        <div className="bg-red-600 p-8 rounded-[32px] text-white shadow-xl shadow-red-200 relative overflow-hidden">
          ...
          <p className="text-xs text-red-50 leading-relaxed font-medium italic">
            Dilarang membuka tab lain atau mematikan koneksi internet selama ujian berlangsung. Pelanggaran akan terdeteksi otomatis oleh sistem.
          </p>
        </div>
      </div>
    </div>
